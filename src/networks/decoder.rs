...
impl<B: Backend> Decoder<B> {
    pub fn init(
        ...
        Self {
            ...
            deconv3: ConvTranspose2dConfig::new([64, 32], [4, 4])
                ...
                .with_padding([1, 1])
                .init(device),
            deconv4: ConvTranspose2dConfig::new([32, image_channels], [4, 4])
                .with_stride([2, 2])
                .with_padding([1, 1])
                .init(device),
            flat_dim,
    ...
    /// input: [B, latent_dim]   (deter + stoch concatenated)
    /// output: [B, C, H, W]  —  reconstructed image in [0, 1]
    pub fn forward(&self, input: Tensor<B, 2>) -> Tensor<B, 4> {
        ...
        sigmoid(self.deconv4.forward(x))
    }
}
